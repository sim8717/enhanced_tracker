...
class Track:
    ...
    ##################### xywh에서 tlwh로 변환 ####################
        # ret = self.mean[:4].copy()
        # ret[:2] -= ret[2:] / 2  # Convert from center (x, y) to top-left (x, y)
        # return ret

    ####################### xyas에서 tlwh로 변환 ####################
        # center_x, center_y, aspect_ratio, area = self.mean[:4]  # Extract the xyas values
        ...
        # # Return in tlwh format
        # return np.array([top_left_x, top_left_y, width, height])


    ####################### xysh에서 tlwh로 변환 ####################
        # center_x, center_y, area, height = self.mean[:4]  # Extract the xysh values
        # # Calculate width from area and height
        # width = area / height
        # # Convert center (x, y) to top-left (x, y)
        # top_left_x = center_x - width / 2
        # top_left_y = center_y - height / 2
    ...
    def update(self, detection, class_id, conf):
        ...
        # logging.debug(f"Track {self.track_id}: Updating state.")##################
        self.conf = conf
        ...
        # self.mean, self.covariance = self.kf.update(self.mean, self.covariance, detection.to_xywh(), detection.confidence)############### 0820 수정
        # self.mean, self.covariance = self.kf.update(self.mean, self.covariance, detection.to_xyas(), detection.confidence)############### 0821 수정
        # self.mean, self.covariance = self.kf.update(self.mean, self.covariance, detection.to_xysh(), detection.confidence)############### 0826 수정
