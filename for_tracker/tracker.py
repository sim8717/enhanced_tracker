...
class Tracker:
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        # msrs = np.asarray([dets[i].to_xyas() for i in detection_indices])############0821 수정
        msrs = np.asarray([dets[i].to_xyah() for i in detection_indices])############0820 수정
        # msrs = np.asarray([dets[i].to_xywh() for i in detection_indices])
        # msrs = np.asarray([dets[i].to_xysh() for i in detection_indices])
    ...
    def _initiate_track(self, detection, class_id, conf):
        self.tracks.append(Track(
            ...
            # detection.to_xywh(), self._next_id, class_id, conf, self.n_init, self.max_age, self.ema_alpha,################# 0820 width로 변경 위해 변경
            # detection.to_xyas(), self._next_id, class_id, conf, self.n_init, self.max_age, self.ema_alpha,################# 0821 면적로 변경 위해 변경
            # detection.to_xysh(), self._next_id, class_id, conf, self.n_init, self.max_age, self.ema_alpha,################# 0826 면적높이로 변경 위해 변경
            detection.feature))
        self._next_id += 1
